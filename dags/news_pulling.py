...
with DAG(
        ...
        description='news pulling, config: {"sources":  "targets": "notion", '
                    '"dedup": true}',
        schedule_interval=timedelta(minutes=3),  # At minute 15 every hour
    ...
    t3 = BashOperator(
        task_id='pull',
        bash_command='cd ~/airflow/run/news-summary/src && python3 af_pull.py '
                     '--start {{ ds }} '
                     '--prefix=./run '
                     '--run-id={{ run_id }} '
                     '--job-id={{ ti.job_id }} '
    ...
    t4 = BashOperator(
        task_id='save',
        bash_command='cd ~/airflow/run/news-summary/src && python3 af_save.py '
                     '--start {{ ds }} '
                     '--prefix=./run '
                     '--run-id={{ run_id }} '
                     '--job-id={{ ti.job_id }} '
