...
with DAG(
    ...
    t3 = BashOperator(
        task_id='pull',
        bash_command='cd ~/airflow/run/news-summary/src && python3 af_sync.py '
                     '--start {{ ds }} '
                     '--prefix=./run '
                     '--run-id={{ run_id }} '
                     '--job-id={{ ti.job_id }} '
    ...
    t4 = BashOperator(
        task_id='save',
        bash_command='cd ~/airflow/run/news-summary/src && python3 af_dist.py '
                     '--start {{ ds }} '
                     '--prefix=./run '
                     '--run-id={{ run_id }} '
                     '--job-id={{ ti.job_id }} '
    ...
    t5 = BashOperator(
        task_id='clean',
        bash_command='cd ~/airflow/run/news-summary/src && python3 af_clean.py '
                     '--start {{ ds }} '
                     '--prefix=./run '
                     '--run-id={{ run_id }} '
                     '--job-id={{ ti.job_id }} '
